...
pub fn export_matched_events(path: &Path, events: &[MatchedEventRecord]) -> anyhow::Result<()> {
    let mut wtr = Writer::from_path(path)?;
    for event in events {
        wtr.serialize(event)?;
    }
...
pub fn export_feature_trace(path: &Path, trace: &[FeatureTraceRecord]) -> anyhow::Result<()> {
    let mut wtr = Writer::from_path(path)?;
    for record in trace {
